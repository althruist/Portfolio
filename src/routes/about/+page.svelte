<script>
  ...
  onMount(() => {
    ...
    mediaQuery.addEventListener("change", onMobile);

    updateThemeColors();

    if (isMobile()) {
      document.querySelectorAll(".socialText").forEach((socialText) => {
...
<title>althruist : about</title>
<div id="content">
  <div id="bigCard" class="card noBounce">
    <h1>Contact me!</h1>
    <div class="socials" bind:this={socials}>
      ...
      <div id="socialGrid">
        ...
        <p class="socialText">Audiotool</p>
      </div>
      <div id="socialGrid">
        <a
          aria-label="YouTube"
